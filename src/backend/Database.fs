...
let private parseOdtFromDb (s: string) =
    ...
    if r.Success then
        ...
        failwith $"Corrupt OffsetDateTime in database: {s}"
...
let getOverlappingBookings (conn: SqliteConnection) (startInstant: Instant) (endInstant: Instant) : Booking list =
    Db.newCommand
        """
        SELECT id, participant_name, participant_email, participant_phone,
               title, description, start_time, end_time, duration_minutes,
...
let private parseInstantFromDb (s: string) =
    ...
    if r.Success then
        ...
        failwith $"Corrupt Instant in database: {s}"
