...
let private makeCachedEvent (id: Guid) (summary: string) (startInstant: Instant) (endInstant: Instant) (isAllDay: bool) : CachedEvent =
    { Id = id
...
let private makeBooking (id: Guid) (title: string) (participantName: string) (startOdt: OffsetDateTime) (endOdt: OffsetDateTime) : Booking =
    { Id = id
...
let private makeAvailabilitySlot (id: Guid) (dayOfWeek: IsoDayOfWeek) (startHour: int) (endHour: int) : HostAvailabilitySlot =
    { Id = id
      DayOfWeek = dayOfWeek
...
let calendarViewTests =
    testList "CalendarView" [
        testList "cachedEventToDto" [
        ...
        testList "expandAvailabilitySlots" [
        ...
        testList "buildCalendarViewEvents" [
            test "merges all event types" {
                ...
                let cachedEvents =
                    [ makeCachedEvent cachedId "External Meeting" (Instant.FromUtc(2026, 2, 16, 15, 0)) (Instant.FromUtc(2026, 2, 16, 16, 0)) false ]
                ...
                let events = buildCalendarViewEvents nyTz rangeStart rangeEnd cachedEvents bookings slots
                ...
                let availabilityEvents = events |> List.filter (fun e -> e.EventType = "availability")
            ...
            test "handles only cached events" {
                ...
                let rangeEnd = Instant.FromUtc(2026, 2, 17, 5, 0)
                let cachedEvents =
                    [ makeCachedEvent id "Meeting" (Instant.FromUtc(2026, 2, 16, 15, 0)) (Instant.FromUtc(2026, 2, 16, 16, 0)) false ]
