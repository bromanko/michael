...
**Source:** `docs/design.md`, `docs/plans/plan-booking-page.md`, `docs/spike-conversational-parser.md`, and existing codebase (`src/frontend/booking/`, `src/backend/`)
**Date:** 2026-02-15
...
| ID | Requirement | Verification |
|----|-------------|--------------|
| EML-010 | If the confirmation email fails to send, then the booking system shall log the error but not cancel or roll back the booking. The participant sees a confirmed booking regardless of email delivery. | Test: Simulate SMTP failure during booking; verify the booking remains confirmed in the database and HTTP 200 is returned. |
| EML-011 | If SMTP is not configured, then the booking system shall skip email sending without error and proceed normally. | Test: Create a booking without SMTP configuration; verify booking succeeds with no email-related errors. |

### Ubiquitous Requirements

| ID | Requirement | Verification |
|----|-------------|--------------|
| EML-020 | The `.ics` attachment shall use Content-Type `text/calendar; charset=utf-8; method={METHOD}` and Content-Disposition `inline; filename="invite.ics"`. | Test: Inspect the MIME structure of a notification email; verify the Content-Type parameters and Content-Disposition. |
| EML-021 | The `.ics` DTSTART and DTEND shall use UTC format (`YYYYMMDDTHHMMSSZ`) without TZID references or VTIMEZONE blocks. | Test: Verify the serialized `.ics` contains `Z`-suffixed timestamps and does not contain `TZID=` or `VTIMEZONE`. |
| EML-022 | The `.ics` PRODID shall be `-//Michael//Michael//EN`. | Test: Verify the serialized `.ics` contains the expected PRODID. |
...
| ID | Requirement | Verification |
|----|-------------|--------------|
| NCF-001 | When SMTP is configured, the booking system shall require the `MICHAEL_PUBLIC_URL` environment variable. The value must start with `http://` or `https://` and must not be empty after the scheme. Trailing slashes shall be stripped. | Test: Start the application with SMTP configured but `MICHAEL_PUBLIC_URL` missing; verify a startup crash with a clear error message. Test with `https://cal.example.com/`; verify the trailing slash is stripped. |
| NCF-002 | When SMTP is configured, the booking system shall require the `MICHAEL_HOST_EMAIL` environment variable. The value must be a valid email address. | Test: Start with SMTP configured but `MICHAEL_HOST_EMAIL` missing; verify a startup crash. Test with an invalid address; verify a startup crash. |
| NCF-003 | The `MICHAEL_HOST_NAME` environment variable shall be optional, defaulting to `"Host"`. | Test: Start without `MICHAEL_HOST_NAME`; verify the default is used. |
| NCF-004 | When SMTP is not configured, the booking system shall not require `MICHAEL_PUBLIC_URL` or `MICHAEL_HOST_EMAIL`. | Test: Start without any SMTP or notification env vars; verify the application starts successfully. |
...
| ID | Requirement | Verification |
|----|-------------|--------------|
| CTK-001 | The bookings table shall have a `cancellation_token` column of type TEXT, nullable for historical bookings created before the feature was added. | Test: Insert a booking with a cancellation token; read it back; verify the token round-trips correctly. |
| CTK-002 | The bookings table shall have a partial unique index on `cancellation_token` where the token is not NULL, preventing token collisions. | Test: Attempt to insert two bookings with the same cancellation token; verify the second insert fails. |
